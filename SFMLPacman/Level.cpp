...
const sf::Vector2f invalidPosition(-1.0f, -1.0f);
const int Level::EntityLayer = 2;

Level::Level(int levelIndex, Game* game)
{
	srand(354668);

	_score = game._score;

	_entities = game->_entities;
...
Level::~Level(void)
{
	...
	for (std::vector<std::vector<Tile*>>::iterator it = _tiles->begin(); it != _tiles->end(); it++)
	{
	...
	for (std::vector<Edible*>::iterator it = _edibles.begin(); it != _edibles.end(); it++)
	{
		delete *it;
	}
